            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                {featuredPosts.map((post) => (
                  ...
                  >
                    <div className="relative bg-gradient-to-br from-gray-900 via-black to-gray-900 rounded-xl p-8 lg:p-10 border border-cyan-400/20 hover:border-cyan-400/50 hover:shadow-2xl hover:shadow-cyan-400/20 transition-all duration-500 overflow-hidden h-full">
                      {/* Animated Accent Bar */}
                      ...
                      {/* Featured Image */}
                      {post.image && (
                        <div className="relative w-full h-48 mb-6 rounded-lg overflow-hidden border border-cyan-400/20">
                      ...
                      <div className="relative z-10 pl-6">
                        {/* Category */}
                        <div className="mb-4">
                          <span className="text-cyan-400 text-xs font-mono uppercase tracking-wider border border-cyan-400/30 px-3 py-1 rounded-full">
                            {post.category}
                          </span>
                        </div>
                        ...
                        <div className="mt-6 flex flex-wrap gap-2">
                          {post.tags.slice(0, 3).map((tag) => (
                            <span
                              key={tag}
                              className="text-xs text-cyan-400/80 font-mono uppercase tracking-wider border border-cyan-400/20 px-2 py-1 rounded"
                            >
                              {tag}
                            </span>
                          ))}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-7xl mx-auto">
            <div className="mb-12 text-center">
              ...
              <button
                onClick={loadPosts}
                className="mt-4 px-4 py-2 text-sm text-cyan-400 hover:text-cyan-300 border border-cyan-400/30 rounded-lg hover:border-cyan-400/50 transition-colors"
              >
                Refresh Posts
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {allPosts.map((post) => (
                <Link
                  ...
                  className="group block"
                >
                  <div className="relative bg-gradient-to-br from-gray-800/80 via-gray-900/90 to-black rounded-xl p-6 lg:p-8 border border-cyan-400/20 hover:border-cyan-400/50 hover:shadow-xl hover:shadow-cyan-400/10 transition-all duration-300 overflow-hidden h-full">
                    {/* Animated Accent Bar */}
                    ...
                    {/* Featured Image */}
                    {post.image && (
                      <div className="relative w-full h-40 mb-4 rounded-lg overflow-hidden border border-cyan-400/20">
                        {post.image.startsWith('data:') ? (
                          <img
                            src={post.image}
                            alt={post.title}
                    ...
                    <div className="relative z-10 pl-6">
                      {/* Category */}
                      <div className="mb-3">
                        <span className="text-cyan-400 text-xs font-mono uppercase tracking-wider border border-cyan-400/30 px-2 py-1 rounded-full">
                          {post.category}
                        </span>
                      </div>
